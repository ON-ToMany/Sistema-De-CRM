...
export class OportunidadeService {
    ...
    async buscarTodas(): Promise<OportunidadeEntity[]> {
        ...
        oportunidades.forEach(oportunidade => {this.processarOportunidade(oportunidade)});
    ...
    async buscarPorEquipamento(nome:string): Promise<OportunidadeEntity[]>{
        ...
        //para exibir os campos de estado de conservação e co2 economizado (pois são apenas de retorno)
        buscarOportunidade.forEach(oportunidade => {this.processarOportunidade(oportunidade)});
    ...
    async finalizar(id: number) {
        ...
        if (oportunidade.status !== StatusOportunidade.PENDENTE) {
            throw new HttpException('Apenas oportunidades pendentes podem ser finalizadas!', HttpStatus.BAD_REQUEST);
        }
    ...
    async cancelar(id: number) {
        ...
        if (oportunidade.status !== StatusOportunidade.PENDENTE) {
            throw new HttpException('Apenas oportunidades pendentes podem ser canceladas!', HttpStatus.BAD_REQUEST);
        }
    ...
    async atualizar(oportunidade: OportunidadeEntity): Promise<OportunidadeEntity>{
        ...
        const oportunidadeAtualizada = await this.oportunidadeRepository.save(oportunidade);
